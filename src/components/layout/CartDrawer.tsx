      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed right-0 top-0 z-[80] flex h-full w-full max-w-md flex-col bg-[--color-ivory] shadow-2xl"
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {lines.length === 0 ? (
                <p className="mt-10 text-center text-sm text-black/60">
                  Your bag is empty. Discover pieces you&apos;ll love.
                </p>
              ) : (
                <ul className="space-y-6">
                  {lines.map((l) => (
                    <li key={l.variantId} className="flex gap-4">
                      <div className="relative h-24 w-20 shrink-0 overflow-hidden rounded-md bg-[--color-beige]">
                        {l.image && (
            ...
            {lines.length > 0 && (
              <div className="border-t border-black/10 px-6 py-5">
                ...
                <Link
                  href="/checkout"
                  onClick={close}
                  className="block w-full rounded-full bg-[--color-charcoal] py-3 text-center text-sm tracking-wide text-white transition hover:bg-[--color-espresso]"
                >
                  Checkout
                </Link>
              </div>
