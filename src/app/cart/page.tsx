      ...
      {lines.length === 0 ? (
        ...
        <div className="grid gap-12 md:grid-cols-[1fr_320px]">
          <ul className="space-y-6">
            {lines.map((l) => (
              <li key={l.variantId} className="flex gap-4 border-b border-black/10 pb-6">
                <div className="relative h-32 w-24 shrink-0 overflow-hidden rounded-lg bg-[--color-beige]">
                  {l.image && <Image src={l.image} alt={l.name} fill className="object-cover" />}
                </div>
                <div className="flex flex-1 flex-col">
                  <div className="flex justify-between">
                    <p className="font-display">{l.name}</p>
                    <button onClick={() => removeLine(l.variantId)} aria-label="Remove">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="flex items-center gap-3 rounded-full border border-black/15 px-3 py-1.5">
                      <button onClick={() => updateQuantity(l.variantId, l.quantity - 1)} aria-label="Decrease">
                        <Minus className="h-3.5 w-3.5" />
                      </button>
                      <span className="w-5 text-center text-sm">{l.quantity}</span>
                      <button onClick={() => updateQuantity(l.variantId, l.quantity + 1)} aria-label="Increase">
          ...
          <div className="h-fit rounded-2xl border border-black/10 p-6">
            ...
            <Link
              href="/checkout"
              className="mt-6 block w-full rounded-full bg-[--color-charcoal] py-3.5 text-center text-sm text-white hover:bg-[--color-espresso]"
            >
              Proceed to Checkout
            </Link>
          </div>
